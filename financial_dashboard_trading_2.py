...
with st.expander("MACD(異同移動平均線)"):
    ...
    fig4.add_trace(go.Scatter(x=KBar_df['Time'][last_nan_index_MACD+1:], y=KBar_df['MACD'][last_nan_index_MACD+1:], mode='lines',line=dict(color='pink', width=2), name='DIF'), 
                  secondary_y=True)
    # 在黃金交叉處添加
    fig4.add_trace(go.Scatter(x=KBar_df.loc[golden_cross_points, 'Time'], y=KBar_df.loc[golden_cross_points, 'Close'], mode='markers', marker=dict(color='red', symbol='triangle-down',size=13), name='黃金交叉'), secondary_y=True)
     
    fig4.add_trace(go.Scatter(x=KBar_df.loc[death_cross_points, 'Time'], y=KBar_df.loc[death_cross_points, 'Close'], mode='markers', marker=dict(color='blue', symbol='triangle-up',size=13), name='死亡交叉'), secondary_y=True)
